...
pair<SourceMap, size_t> TestCaseReader::parseSourcesAndSettingsWithLineNumber(istream& _stream)
{
	...
	vector<pair<string, boost::filesystem::path>> externalSources;
	...
	while (getline(_stream, line))
	{
		...
		else if (sourcePart)
		{
			...
			else if (boost::algorithm::starts_with(line, externalSourceDelimiterStart) && boost::algorithm::ends_with(line, sourceDelimiterEnd))
			{
				...
				fs::path externalSourceTarget(fs::path{externalSourceString});
				fs::path testCaseParentDir = m_fileName.parent_path();
				if (!fs::path(externalSourceTarget).is_relative())
				...
				if (!fs::exists(externalSourceFullPath))
					BOOST_THROW_EXCEPTION(runtime_error("External Source '" + externalSourceTarget.generic_string() + "' not found."));
				else
				...
				externalSources.emplace_back(externalSourceName, externalSourceTarget);
