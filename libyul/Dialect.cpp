...
Literal Dialect::zeroLiteralForType(solidity::yul::YulString _type) const
{
	if (_type == boolType && _type != defaultType)
		return {createDebugData(), LiteralKind::Boolean, "false"_yulstring, _type};
	return {createDebugData(), LiteralKind::Number, "0"_yulstring, _type};
}
...
Literal Dialect::trueLiteral() const
{
	if (boolType != defaultType)
		return {createDebugData(), LiteralKind::Boolean, "true"_yulstring, boolType};
	else
		return {createDebugData(), LiteralKind::Number, "1"_yulstring, defaultType};
}
