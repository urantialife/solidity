...
inline std::shared_ptr<DebugData const> createDebugData(langutil::SourceLocation _location = {})
{
	return std::make_shared<DebugData const>(_location);
}

inline void changeDebugDataLocationEnd(std::shared_ptr<DebugData const>& io_debugData, int _end)
{
	langutil::SourceLocation loc = io_debugData->location;
	loc.end = _end;
	io_debugData = std::make_shared<DebugData const>(loc);
}
...
{
	template <class T> std::shared_ptr<DebugData const> operator()(T const& _node) const
	{
